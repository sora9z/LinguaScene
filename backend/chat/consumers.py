...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_initial_message(self):
        if await self.get_message_count(self.room) == 0:
            ...
            response_message = await self.openai_service.get_initial_response(
                initial_setting
            )
            initial_system_message, response = (
            ...
            self._append_to_message_buffer(
                type=MessageType.REQUEST_SYSTEM_MESSAGE, message=initial_system_message
            )
            self._append_to_message_buffer(
                type=MessageType.RESPONSE_ASSISTANT_MESSAGE, message=response
            )
        else:
    ...
    async def handle_user_message(self, text_data_json):
        ...
        assistant_message: GptMessage = await self.get_response(
        ...
        self._append_to_message_buffer(
            type="assistant_message", message=assistant_message
        )
    ...
    async def get_response(self, message_type: str) -> GptMessage:
        ...
        try:
            if message_type == MessageType.REQUEST_USER_MESSAGE:
                ...
                raise Exception(
                    "[consumer/get_response] Invelid message type"
                )  # log로 남기기 예외는 필요할까?
    ...
    def _append_to_message_buffer(self, type, message: GptMessage):
        if (
            type == MessageType.REQUEST_USER_MESSAGE
            or type == MessageType.RESPONSE_ASSISTANT_MESSAGE
        ):
    ...
    def save_message(self, message: GptMessage):
        msg = Message.objects.create(
            chat_room=self.room, role=message["role"], content=message["content"]
        )
        msg.save()
